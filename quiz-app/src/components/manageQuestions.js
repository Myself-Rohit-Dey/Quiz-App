...
const ManageQuestions = () => {
  ...
  const [answers, setAnswers] = useState([{ id: 1, text: '', isCorrect: false }]);
    ...
    try {
      const response = await fetch('https://7406-2402-3a80-196b-fc9b-44eb-26a5-afab-83ef.ngrok-free.app/admin/set-question', {
        ...
        body: JSON.stringify({
          ...
          answer_id: String.fromCharCode(answers.find(answer => answer.isCorrect)?.id + 96) || 'a', // Get ID of the correct answer
          answer_text: answers.find(answer => answer.isCorrect)?.text || '',
          explanation: explanation,
          difficulty: difficulty,
          marks: marks,
          ...answers.reduce((acc, answer, index) => ({ ...acc, [`op${index + 1}`]: answer.text }), {}) // Add options
        }),
      });
      if (!response.ok) {
  ...
  const handleAddAnswer = () => {
    setAnswers(prevAnswers => [...prevAnswers, { id: prevAnswers.length + 1, text: '', isCorrect: false }]);
  };

  const handleAnswerChange = (text, id) => {
    setAnswers(prevAnswers => prevAnswers.map(answer => answer.id === id ? { ...answer, text: text } : answer));
  };

  const handleToggleCorrect = (id) => {
    setAnswers(prevAnswers =>
      prevAnswers.map(answer =>
        answer.id === id ? { ...answer, isCorrect: !answer.isCorrect } : { ...answer, isCorrect: false }
      )
    );
  };
    ...
    <View style={styles.container}>
      ...
      {questions.map((question, index) => (
        <View key={index} style={styles.questionContainer}>
          ...
          <Text style={styles.answer}>Correct Answer: {question.answer_text}</Text>
        </View>
      ))}
      <Modal
        animationType="slide"
              ...
              renderItem={({ item }) => (
                <View style={styles.answerContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.toggleCorrectButton, { backgroundColor: item.isCorrect ? 'green' : 'gray' }]}
                  >
                    <Text style={styles.toggleCorrectText}>{item.isCorrect ? 'Correct' : 'Incorrect'}</Text>
                  </TouchableOpacity>
                </View>
              )}
